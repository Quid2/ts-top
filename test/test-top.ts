// NOTE: this test must be run into a browser (needs WebSocket)
import { flat, typedBLOB } from "../ts/index";
import { Channel, channel } from "@quid2/ts/top/socket";
import { Bit, V0, V1, $Bit } from "@quid2/ts/ADT/Bit/K65149ce3b366";

export function testTop() {
  var chan0 = new Channel($Bit, {
  ...
  var chan = new Channel($Bit, {
    onOpen: function (send, close) {
      for (let v of [new V1(), new V0(), new V1(), new V1(), new V1()]) {
        console.log(v);
        send(v);
      }
      close();
...
function testTopRX() {
  ...
  function log(n: string, v: Bit): void {
    received.push([n + ":" + v.toString()]);
  }
  ...
  setTimeout(() => {
    console.log(
      "Received",
      received.toString(),
      received.toString() == "2.1:V0,1.1:V1,1.2:V1"
    );
