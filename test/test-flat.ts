...
// Check roundabout
function flatRound<A extends Flat>(typ: zmFold<A>, v: A) {
    log("");
    log("Val:");
    log(v)
    log(v.toString());
...
function testFlat() {
    ...
    flatRound($ChannelSelectionResult($WebSocketAddress($IP4Address)), new Success);
    ...
    flatRound($NonEmptyList($Bit), new NECons(new V0, new NECons(new V1, new Elem(new V0))));
