...
export function channel<A>(t: zmFold<A>): [Observable<A>, QueueingSubject<A>] {
  ...
  const inChan = new Observable<A>((observer) => {
    ...
    socket.onclose = (event: CloseEvent) => {
      ...
      if (event.wasClean) observer.complete();
      else observer.error(new Error(event.reason));
    };
  ...
  constructor(inType: zmFold<I>, outType: zmFold<R>, ms: number = 7000) {
    ...
    this.inChan
      ...
      .subscribe(function (r) {
        ...
        self.calls.use(key, function (f: any) {
          f(r._1);
        });
      });
  }
...
function testCall() {
  const ch = new CallChannel(
    $Validate($SourceCode($ZM)),
    $List($Note($String, $Range))
  );
  //assert.fail("BAD")
  const results = [
    [
  ...
  for (let i = 0; i < results.length; i++) {
    let rs = results[i];
    ch.call(new Validate(new SourceCode(new ZML(), new String(rs[0])))).then(
      function (r) {
...
function testRX() {
  var observable: Observable<number> = Observable.create(function (
...
import * as __test from "tape"
    ...
    __test("flatBLOB", t => {
      ...
      t.deepEqual(flat(3), [233,1], "flat(3) == [233,1]")
