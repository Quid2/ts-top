...
export const $BLOB: <A extends Q.ZM>(
  t0: Q.zmFold<A>
) => Q.zmFold<BLOB<A>> = function (t1) {
  return function (f) {
    return f(___, [t1(f)]);
  };
};
...
export class BLOB<A extends Q.ZM> implements Q.ZM {
  ...
  toStr(nested = false): string {
    return Q.nestedPars(
      nested,
      ["BLOB", this.encoding.toStr(true), this.content.toStr(true)].join(" ")
    );
  }
  pretty(nested = false): string {
    return Q.nestedPars(
      nested,
      ["BLOB", this.encoding.pretty(true), this.content.pretty(true)].join(" ")
    );
  }

  match<R>(m: { BLOB: (v0: A, v1: Kf8844385a443.Bytes) => R }): R {
