...
export const $UnicodeLetterOrNumberOrLine: Q.zmFold<UnicodeLetterOrNumberOrLine> = function (
  f
) {
  return f(___, []);
};
...
export class UnicodeLetterOrNumberOrLine implements Q.ZM {
  ...
  toStr(nested = false): string {
    return Q.nestedPars(
      nested,
      ["UnicodeLetterOrNumberOrLine", this._0.toStr(true)].join(" ")
    );
  }
  pretty(nested = false): string {
    return Q.nestedPars(
      nested,
      ["UnicodeLetterOrNumberOrLine", this._0.pretty(true)].join(" ")
    );
  }
