...
export const $Position: Q.zmFold<Position> = function (f) {
  return f(___, []);
};
...
export class Position implements Q.ZM {
  ...
  toStr(nested = false): string {
    return Q.nestedPars(
      nested,
      ["Position", this.row.toStr(true), this.column.toStr(true)].join(" ")
    );
  }
  pretty(nested = false): string {
    return Q.nestedPars(
      nested,
      ["Position", this.row.pretty(true), this.column.pretty(true)].join(" ")
    );
  }
