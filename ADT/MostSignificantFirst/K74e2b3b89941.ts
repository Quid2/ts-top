...
export const $MostSignificantFirst: <A extends Q.ZM>(
  t0: Q.zmFold<A>
) => Q.zmFold<MostSignificantFirst<A>> = function (t1) {
  return function (f) {
    return f(___, [t1(f)]);
  };
};
...
export class MostSignificantFirst<A extends Q.ZM> implements Q.ZM {
  ...
  toStr(nested = false): string {
    return Q.nestedPars(
      nested,
      ["MostSignificantFirst", this._0.toStr(true)].join(" ")
    );
  }
  pretty(nested = false): string {
    return Q.nestedPars(
      nested,
      ["MostSignificantFirst", this._0.pretty(true)].join(" ")
    );
  }

  match<R>(m: { MostSignificantFirst: (v0: A) => R }): R {
