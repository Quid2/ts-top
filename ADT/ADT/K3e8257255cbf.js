...
class ADT {
    ...
    toStr(nested = false) { return Q.nestedPars(nested, ["ADT", this.declName.toStr(true), this.declNumParameters.toStr(true), this.declCons.toStr(true)].join(' ')); }
    match(m) { return m.ADT(this.declName, this.declNumParameters, this.declCons); }
    flatMaxSize() { return this.declName.flatMaxSize() + this.declNumParameters.flatMaxSize() + this.declCons.flatMaxSize(); }
    flatEncode(st) { this.declName.flatEncode(st); this.declNumParameters.flatEncode(st); this.declCons.flatEncode(st); }
}
exports.ADT = ADT;
