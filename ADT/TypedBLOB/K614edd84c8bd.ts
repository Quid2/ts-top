...
export const $TypedBLOB: Q.zmFold<TypedBLOB> = function (f) {
  return f(___, []);
};
  ...
  decoder: function (decoders) {
    return function (st) {
      return new TypedBLOB(
        ...
        Kf139d4751fda.___.decoder([K982148c09ddb.___.decoder([])])(st)
      );
    };
  },
};
...
export class TypedBLOB implements Q.ZM {
  ...
  toStr(nested = false): string {
    return Q.nestedPars(
      nested,
      ["TypedBLOB", this._0.toStr(true), this._1.toStr(true)].join(" ")
    );
  }
  pretty(nested = false): string {
    return Q.nestedPars(
      nested,
      ["TypedBLOB", this._0.pretty(true), this._1.pretty(true)].join(" ")
    );
  }
  ...
  match<R>(m: {
    ...
    return m.TypedBLOB(this._0, this._1);
  }
  flatMaxSize(): number {
