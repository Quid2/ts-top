...
class WebSocketAddress {
    ...
    toStr(nested = false) { return Q.nestedPars(nested, ["WebSocketAddress", this.secure.toStr(true), this.host.toStr(true), this.path.toStr(true)].join(' ')); }
    match(m) { return m.WebSocketAddress(this.secure, this.host, this.path); }
    flatMaxSize() { return this.secure.flatMaxSize() + this.host.flatMaxSize() + this.path.flatMaxSize(); }
    flatEncode(st) { this.secure.flatEncode(st); this.host.flatEncode(st); this.path.flatEncode(st); }
}
