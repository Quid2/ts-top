...
export const $Content: <A extends Q.ZM, B extends Q.ZM>(
  t0: Q.zmFold<A>,
  t1: Q.zmFold<B>
) => Q.zmFold<Content<A, B>> = function (t1, t2) {
  return function (f) {
...
export type Content<A extends Q.ZM, B extends Q.ZM> =
  | TextMessage<A, B>
  | AskSubSubjects<A, B>
  | Join<A, B>
  | Leave<A, B>
...
export class TextMessage<A extends Q.ZM, B extends Q.ZM> implements Q.ZM {
  ...
  pretty(nested = false): string {
    return Q.nestedPars(
      nested,
      ["TextMessage", this._0.pretty(true)].join(" ")
    );
  }
...
export class Users<A extends Q.ZM, B extends Q.ZM> implements Q.ZM {
  constructor(public readonly _0: Kb8cd13187198.List<A>) {}
  ...
  pretty(nested = false): string {
    return Q.nestedPars(nested, ["Users", this._0.pretty(true)].join(" "));
  }
...
export class History<A extends Q.ZM, B extends Q.ZM> implements Q.ZM {
  constructor(public readonly _0: Kb8cd13187198.List<B>) {}
  ...
  pretty(nested = false): string {
    return Q.nestedPars(nested, ["History", this._0.pretty(true)].join(" "));
  }
  ...
  match<R>(m: {
    ...
    return m.History(this._0);
  }
  flatMaxSize(): number {
    return 4 + this._0.flatMaxSize();
  }
  flatEncode(st: Q.EncoderState) {
    st.one();
    st.one();
